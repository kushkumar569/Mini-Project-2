...
const { Teacher } = require("../DataBase/Account.cjs")
...
Get.get("/getTeacher", async (req, res) => {
    try {
        const result = await Teacher.find({});
        if (!result) {
            return res.status(404).json({ message: "Teacher Not Matched" })
        }
        return res.status(200).json(result);
    } catch (err) {
        return res.status(500).json({ error: error.message })
...
Get.get("/getCourseCode", async (req,res)=>{
    try {
        const result = await subject.find({});
        if (!result) {
            return res.status(404).json({ message: "CourseCode Not Matched" })
        }
        return res.status(200).json(result);
    } catch (err) {
        return res.status(500).json({ error: error.message })
...
Get.get("/getLocation", async (req,res)=>{
    try{
...
Get.get("/getSchedule",async (req,res)=>{
    try{
